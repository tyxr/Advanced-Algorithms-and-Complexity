...
public class Diet {
    ...
    int solveDietProblem(int n, int m, double A[][], double[] b, double[] c, double[] x) {
        ...
        for (int[] Current_Combination : combinations) {
            new_a = new double[m][m];
            new_b = new double[m];
            cutRow(A, b, new_a, new_b, Current_Combination);
            double[] solution;
            try {
                solution = gaussianElimination(new_a, new_b);
            } catch (IllegalArgumentException e) {
                continue;
            }
            int[] currentComplement = complementOfIndexes(IntStream.range(0, n + m + 1).toArray(), Current_Combination);
            new_a = new double[currentComplement.length][m];
            new_b = new double[currentComplement.length];
            cutRow(A, b, new_a, new_b, currentComplement);
            if (checkSolution(solution, new_a, new_b)) {
    ...
    public static void cutRow(double[][] A, double[] b, double[][] new_a, double[] new_b, int[] indexes) {
        for (int i = 0; i < indexes.length; i++) {
            if (indexes[i] < A.length) {
                new_a[i] = Arrays.copyOf(A[indexes[i]], A[i].length);
                new_b[i] = b[indexes[i]];
            } else if (indexes[i] == A.length + A[0].length) {
                Arrays.fill(new_a[i], 1);
                new_b[i] = 1_000_000_000D;
            } else {
                new_a[i][indexes[i] - A.length] = -1;
            }
        }
    }
